...
class GuideDialog(QDialog):
    ...
    def _section2(self) -> QVBoxLayout:
        items = [
            ...
            (4, "웹서버 실행 명령 등록 (선택)",
             ...
             "서버 버튼이나 '함께 시작' 옵션으로 터널과 같이 실행할 수 있습니다."),
            (5, "웹서버 실행",
             "터널보다 웹서버를 먼저 켜는 것이 좋습니다. 순서가 반대여도 서버가 뜨면 "
             "자동으로 연결되지만, 그 사이 로그에 연결 거부 메시지가 남습니다."),
            (6, "터널 켜기",
             "카드의 '켜기' 버튼을 누릅니다. 상태등이 초록색으로 바뀌고 로그에 "
             "Registered tunnel connection이 보이면 연결된 것입니다."),
            (7, "접속 확인",
    ...
    def _section3(self) -> QFrame:
        lines = [
            ...
            "127.0.0.1",
            "이 앱을 종료하면 실행 중이던 터널도 함께 꺼집니다. 24시간 운영하려면 "
            "라즈베리파이에 올리고 SSH 원격 모드로 관리하세요.",
