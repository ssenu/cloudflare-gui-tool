...
PUBLISH_STEPS = [
    ...
    ("접속 확인",
     "라우트 줄의 도메인을 누르면 브라우저에서 열립니다. ⋮ → 접속 확인을 쓰면 "
     "앱이 직접 열어 보고 404·502 같은 결과와 다음에 할 일을 알려줍니다."),
    ("부팅 시 자동 실행",
     "터널 카드의 ⋮ → '부팅 시 자동 실행'을 켜면 대상 기기가 재부팅돼도 "
     "systemd가 터널을 다시 띄웁니다. 앱을 켜지 않아도 사이트가 살아 있게 "
     "하려면 이걸 켜 두세요(SSH 대상 전용)."),
]
...
PROJECT_STEPS = [
    ...
    ("배포",
     "코드를 고친 뒤에는 ⋮ → 배포를 누르면 최신 코드를 받고(git pull) 컨테이너를 "
     "다시 띄우는 것까지 한 번에 합니다. 진행 상황은 ⋮ → 로그에서 볼 수 있고, "
     "받아오기만 하려면 '업데이트'를 쓰면 됩니다."),
]
...
NOTES = [
    ...
    "그렇다면 터널을 껐다 켜세요(안내 배너의 '지금 재시작'). 502는 반대로 "
    "라우트는 맞는데 그 포트에 서버가 없다는 뜻입니다.",
