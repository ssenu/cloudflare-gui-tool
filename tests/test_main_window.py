...
def test_deploy_shows_completion_then_auto_hides(qapp, tmp_path):
    """배포 중 문구 -> 웹이 응답하면 '완료되었습니다' -> 5초 뒤 사라짐."""
    import time as _time
    ...
    win.ctx.manager.start_service = lambda t, r: None
    ready = {"on": False}
    win.ctx.manager.service_running = lambda t, r: ready["on"]
    win.ctx.manager.service_pending = lambda t, r: not ready["on"]

    win._deploy_route(card, route)
    deadline = _time.monotonic() + 3
    while win._site_pollers[-1].is_busy() and _time.monotonic() < deadline:
        QApplication.processEvents()
        _time.sleep(0.005)
...
def test_deploy_watch_gives_up_after_deadline(qapp, tmp_path):
    """10분이 지나도 완료가 안 잡히면 감시만 접는다(완료 문구를 띄우지 않는다)."""
    import time as _time
...
def test_new_banner_cancels_pending_auto_hide(qapp, tmp_path):
    """완료 알림 직후 다른 안내가 뜨면, 예전 5초 예약이 새 안내를 지우면 안 된다."""
    win = make_window(qapp, tmp_path)
    win._show_info("완료되었습니다.", auto_hide_ms=5000)
    assert win._info_hide_timer.isActive()

    win._show_info("다른 안내")                 # auto_hide 없음

    assert not win._info_hide_timer.isActive()  # 이전 예약 취소
    assert not win.info_banner.isHidden()
